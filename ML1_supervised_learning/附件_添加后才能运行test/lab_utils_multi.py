...
def compute_gradient(X, y, w, b): 
    ...
    for i in range(m):                             
        ...
        for j in range(n):                         
            dj_dw[j] = dj_dw[j] + err * X[i,j]    #*`X[i,j]`**: 第i个样本的第j个特征值
        dj_db = dj_db + err                        
    dj_dw = dj_dw/m                                
    dj_db = dj_db/m                                
...
def gradient_descent(X, y, w_in, b_in, cost_function, gradient_function, alpha, num_iters): 
    ...
    for i in range(num_iters):# 1. 计算梯度
        ...
        # Save cost J,w,b at each save interval for graphing
        if i == 0 or i % save_interval == 0:     
            hist["cost"].append(cost_function(X, y, w, b)) #cost_function = compute_gradient
            hist["params"].append([w,b])
            hist["grads"].append([dj_dw,dj_db])
            hist["iter"].append(i) #当前迭代次数
...
def load_house_data():
    data = np.loadtxt("/root/DL/supervised_learning/houses.txt", delimiter=',', skiprows=1)
    X = data[:,:4]
    y = data[:,4]
    return X, y
